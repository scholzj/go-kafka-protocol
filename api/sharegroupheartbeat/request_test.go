...
// Round-trips a fully populated ShareGroupHeartbeatRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestShareGroupHeartbeatRequestRoundTrip(t *testing.T) {
	in := &ShareGroupHeartbeatRequest{
		GroupId:              reqPtr("x"),
		MemberId:             reqPtr("x"),
		MemberEpoch:          1,
		RackId:               reqPtr("x"),
	...
	inNulls := &ShareGroupHeartbeatRequest{
		GroupId:              reqPtr("x"),
		MemberId:             reqPtr("x"),
		MemberEpoch:          1,
		RackId:               nil,
	...
	for v := int16(1); v <= 1; v++ {
		{
			...
			out := &ShareGroupHeartbeatRequest{}
		...
		_ = (&ShareGroupHeartbeatRequest{}).PrettyPrint()
	}
}
