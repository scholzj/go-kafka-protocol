...
// Round-trips a fully populated ShareGroupHeartbeatResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestShareGroupHeartbeatResponseRoundTrip(t *testing.T) {
	in := &ShareGroupHeartbeatResponse{
		ThrottleTimeMs:      1,
		ErrorCode:           1,
		...
		Assignment: &ShareGroupHeartbeatResponseAssignment{
			TopicPartitions: &[]ShareGroupHeartbeatResponseAssignmentTopicPartition{ShareGroupHeartbeatResponseAssignmentTopicPartition{
				TopicId:    uuid.UUID{},
				Partitions: &[]int32{1},
			}},
		},
	...
	inNulls := &ShareGroupHeartbeatResponse{
		ThrottleTimeMs:      1,
		ErrorCode:           1,
		ErrorMessage:        nil,
		MemberId:            nil,
	...
	for v := int16(1); v <= 1; v++ {
		{
			...
			out := &ShareGroupHeartbeatResponse{}
		...
		_ = (&ShareGroupHeartbeatResponse{}).PrettyPrint()
	}
}
