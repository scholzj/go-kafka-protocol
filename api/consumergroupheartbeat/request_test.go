...
// Round-trips a fully populated ConsumerGroupHeartbeatRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestConsumerGroupHeartbeatRequestRoundTrip(t *testing.T) {
	in := &ConsumerGroupHeartbeatRequest{
		GroupId:              reqPtr("x"),
		MemberId:             reqPtr("x"),
		MemberEpoch:          1,
		InstanceId:           reqPtr("x"),
		...
		SubscribedTopicRegex: reqPtr("x"),
		ServerAssignor:       reqPtr("x"),
		TopicPartitions: &[]ConsumerGroupHeartbeatRequestTopicPartition{ConsumerGroupHeartbeatRequestTopicPartition{
			TopicId:    uuid.UUID{},
			Partitions: &[]int32{1},
		}},
	}
	...
	inNulls := &ConsumerGroupHeartbeatRequest{
		GroupId:              reqPtr("x"),
		MemberId:             reqPtr("x"),
		MemberEpoch:          1,
		InstanceId:           nil,
	...
	for v := int16(0); v <= 1; v++ {
		{
			...
			out := &ConsumerGroupHeartbeatRequest{}
		...
		_ = (&ConsumerGroupHeartbeatRequest{}).PrettyPrint()
	}
}
