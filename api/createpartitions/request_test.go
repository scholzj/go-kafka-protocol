...
// Round-trips a fully populated CreatePartitionsRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestCreatePartitionsRequestRoundTrip(t *testing.T) {
	in := &CreatePartitionsRequest{
		Topics: &[]CreatePartitionsRequestTopic{CreatePartitionsRequestTopic{
			Name:  reqPtr("x"),
			Count: 1,
			Assignments: &[]CreatePartitionsRequestTopicAssignment{CreatePartitionsRequestTopicAssignment{
				BrokerIds: &[]int32{1},
			}},
		}},
		TimeoutMs:    1,
	...
	inNulls := &CreatePartitionsRequest{
		Topics: &[]CreatePartitionsRequestTopic{CreatePartitionsRequestTopic{
			Name:        reqPtr("x"),
			Count:       1,
			Assignments: nil,
	...
	for v := int16(0); v <= 3; v++ {
		...
		_ = (&CreatePartitionsRequest{}).PrettyPrint()
	}
}
