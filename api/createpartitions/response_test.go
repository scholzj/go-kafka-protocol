...
// Round-trips a fully populated CreatePartitionsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestCreatePartitionsResponseRoundTrip(t *testing.T) {
	in := &CreatePartitionsResponse{
		ThrottleTimeMs: 1,
		Results: &[]CreatePartitionsResponseResult{CreatePartitionsResponseResult{
			Name:         resPtr("x"),
			ErrorCode:    1,
	...
	inNulls := &CreatePartitionsResponse{
		ThrottleTimeMs: 1,
		Results: &[]CreatePartitionsResponseResult{CreatePartitionsResponseResult{
			Name:         resPtr("x"),
			ErrorCode:    1,
	...
	for v := int16(0); v <= 3; v++ {
		...
		_ = (&CreatePartitionsResponse{}).PrettyPrint()
	}
}
