...
// Round-trips a fully populated DescribeShareGroupOffsetsRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeShareGroupOffsetsRequestRoundTrip(t *testing.T) {
	in := &DescribeShareGroupOffsetsRequest{
		Groups: &[]DescribeShareGroupOffsetsRequestGroup{DescribeShareGroupOffsetsRequestGroup{
			GroupId: reqPtr("x"),
			Topics: &[]DescribeShareGroupOffsetsRequestGroupTopic{DescribeShareGroupOffsetsRequestGroupTopic{
				TopicName:  reqPtr("x"),
				Partitions: &[]int32{1},
			}},
		}},
	...
	inNulls := &DescribeShareGroupOffsetsRequest{
		Groups: &[]DescribeShareGroupOffsetsRequestGroup{DescribeShareGroupOffsetsRequestGroup{
			GroupId: reqPtr("x"),
			Topics:  nil,
		}},
	...
	for v := int16(0); v <= 1; v++ {
		{
			...
			out := &DescribeShareGroupOffsetsRequest{}
		...
		_ = (&DescribeShareGroupOffsetsRequest{}).PrettyPrint()
	}
}
