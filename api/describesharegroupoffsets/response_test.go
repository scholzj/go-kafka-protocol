...
// Round-trips a fully populated DescribeShareGroupOffsetsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeShareGroupOffsetsResponseRoundTrip(t *testing.T) {
	in := &DescribeShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		Groups: &[]DescribeShareGroupOffsetsResponseGroup{DescribeShareGroupOffsetsResponseGroup{
			GroupId: resPtr("x"),
			Topics: &[]DescribeShareGroupOffsetsResponseGroupTopic{DescribeShareGroupOffsetsResponseGroupTopic{
				TopicName: resPtr("x"),
				TopicId:   uuid.UUID{},
				Partitions: &[]DescribeShareGroupOffsetsResponseGroupTopicPartition{DescribeShareGroupOffsetsResponseGroupTopicPartition{
					PartitionIndex: 1,
					StartOffset:    1,
					LeaderEpoch:    1,
					Lag:            1,
	...
	inNulls := &DescribeShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		Groups: &[]DescribeShareGroupOffsetsResponseGroup{DescribeShareGroupOffsetsResponseGroup{
			GroupId: resPtr("x"),
			Topics: &[]DescribeShareGroupOffsetsResponseGroupTopic{DescribeShareGroupOffsetsResponseGroupTopic{
	...
	for v := int16(0); v <= 1; v++ {
		{
			...
			out := &DescribeShareGroupOffsetsResponse{}
		...
		_ = (&DescribeShareGroupOffsetsResponse{}).PrettyPrint()
	}
}
