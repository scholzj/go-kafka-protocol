...
func (m *ShareFetchRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		if isFlexible {
			...
			m.Topics = make([]ShareFetchRequestFetchTopic, length)
			for i := int32(0); i < length; i++ {
				...
				if version >= 0 && version <= 999 {
					...
					if isFlexible {
						...
						length = int32(lengthUint - 1)
						m.Topics[i].Partitions = make([]ShareFetchRequestFetchPartition, length)
						for i := int32(0); i < length; i++ {
							// PartitionIndex
							if version >= 0 && version <= 999 {
								val, err := protocol.ReadInt32(r)
							...
							if version >= 0 && version <= 999 {
								...
								if isFlexible {
									...
									length = int32(lengthUint - 1)
									m.Topics[i].Partitions[i].AcknowledgementBatches = make([]ShareFetchRequestAcknowledgementBatch, length)
									for i := int32(0); i < length; i++ {
										// FirstOffset
										if version >= 0 && version <= 999 {
	...
	if version >= 0 && version <= 999 {
		...
		if isFlexible {
			...
			length = int32(lengthUint - 1)
			m.ForgottenTopicsData = make([]ShareFetchRequestForgottenTopic, length)
			for i := int32(0); i < length; i++ {
				// TopicId
				if version >= 0 && version <= 999 {
					val, err := protocol.ReadUUID(r)
