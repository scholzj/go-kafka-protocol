...
// Round-trips a fully populated DescribeLogDirsRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeLogDirsRequestRoundTrip(t *testing.T) {
	in := &DescribeLogDirsRequest{
		Topics: &[]DescribeLogDirsRequestTopic{DescribeLogDirsRequestTopic{
			Topic:      reqPtr("x"),
			Partitions: &[]int32{1},
		}},
	}
	...
	inNulls := &DescribeLogDirsRequest{
		Topics: nil,
	}
	...
	for v := int16(1); v <= 5; v++ {
		...
		_ = (&DescribeLogDirsRequest{}).PrettyPrint()
	}
}
