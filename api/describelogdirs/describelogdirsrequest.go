...
func (m *DescribeLogDirsRequest) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(DescribeLogDirsRequestDescribableLogDirTopic)
			if !ok {
...
func (m *DescribeLogDirsRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem DescribeLogDirsRequestDescribableLogDirTopic
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem DescribeLogDirsRequestDescribableLogDirTopic
				// Topic
				if version >= 0 && version <= 999 {
					if isFlexible {
			...
			m.Topics = make([]DescribeLogDirsRequestDescribableLogDirTopic, len(decoded))
			for i, item := range decoded {
				m.Topics[i] = item.(DescribeLogDirsRequestDescribableLogDirTopic)
			}
		} else {
...
// writeTaggedFields writes tagged fields for DescribeLogDirsRequestDescribableLogDirTopic.
func (m *DescribeLogDirsRequestDescribableLogDirTopic) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for DescribeLogDirsRequestDescribableLogDirTopic.
func (m *DescribeLogDirsRequestDescribableLogDirTopic) readTaggedFields(r io.Reader, version int16) error {
