...
// Round-trips a fully populated DescribeAclsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeAclsResponseRoundTrip(t *testing.T) {
	in := &DescribeAclsResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   resPtr("x"),
		Resources: &[]DescribeAclsResponseResource{DescribeAclsResponseResource{
			ResourceType: 1,
			ResourceName: resPtr("x"),
			PatternType:  1,
			Acls: &[]DescribeAclsResponseResourceAcl{DescribeAclsResponseResourceAcl{
				Principal:      resPtr("x"),
				Host:           resPtr("x"),
				Operation:      1,
				PermissionType: 1,
	...
	for v := int16(1); v <= 3; v++ {
		...
		_ = (&DescribeAclsResponse{}).PrettyPrint()
	}
}
