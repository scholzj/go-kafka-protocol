...
// Round-trips a fully populated DeleteTopicsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDeleteTopicsResponseRoundTrip(t *testing.T) {
	in := &DeleteTopicsResponse{
		ThrottleTimeMs: 1,
		Responses: &[]DeleteTopicsResponseResponse{DeleteTopicsResponseResponse{
			Name:         resPtr("x"),
			TopicId:      uuid.UUID{},
			ErrorCode:    1,
	...
	for v := int16(1); v <= 6; v++ {
		...
		_ = (&DeleteTopicsResponse{}).PrettyPrint()
	}
}
