...
func (req *ShareGroupDescribeRequest) Write(w io.Writer) error {
	// GroupIds (versions: 0+)
	if req.GroupIds == nil {
		return fmt.Errorf("ShareGroupDescribeRequest.GroupIds must not be nil in version %d", req.ApiVersion)
	}
...
func (req *ShareGroupDescribeRequest) Read(request *protocol.Request) error {
...
func (req *ShareGroupDescribeRequest) PrettyPrint() string {
	w := bytes.NewBuffer([]byte{})

	fmt.Fprintf(w, "    -> ShareGroupDescribeRequest:\n")
