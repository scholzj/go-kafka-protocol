...
// Round-trips a fully populated SaslAuthenticateResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestSaslAuthenticateResponseRoundTrip(t *testing.T) {
	in := &SaslAuthenticateResponse{
		ErrorCode:         1,
		ErrorMessage:      resPtr("x"),
		AuthBytes:         &[]byte{1, 2, 3},
		SessionLifetimeMs: 1,
	}
	...
	inNulls := &SaslAuthenticateResponse{
		ErrorCode:         1,
		ErrorMessage:      nil,
		AuthBytes:         &[]byte{1, 2, 3},
		SessionLifetimeMs: 1,
	...
	for v := int16(0); v <= 2; v++ {
		...
		_ = (&SaslAuthenticateResponse{}).PrettyPrint()
	}
}
