...
// Round-trips a fully populated DeleteShareGroupStateResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDeleteShareGroupStateResponseRoundTrip(t *testing.T) {
	in := &DeleteShareGroupStateResponse{
		Results: &[]DeleteShareGroupStateResponseResult{DeleteShareGroupStateResponseResult{
			TopicId: uuid.UUID{},
			Partitions: &[]DeleteShareGroupStateResponseResultPartition{DeleteShareGroupStateResponseResultPartition{
				Partition:    1,
				ErrorCode:    1,
	...
	inNulls := &DeleteShareGroupStateResponse{
		Results: &[]DeleteShareGroupStateResponseResult{DeleteShareGroupStateResponseResult{
			TopicId: uuid.UUID{},
			Partitions: &[]DeleteShareGroupStateResponseResultPartition{DeleteShareGroupStateResponseResultPartition{
				Partition:    1,
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &DeleteShareGroupStateResponse{}
		...
		_ = (&DeleteShareGroupStateResponse{}).PrettyPrint()
	}
}
