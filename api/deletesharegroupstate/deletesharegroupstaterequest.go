...
func (m *DeleteShareGroupStateRequest) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(DeleteShareGroupStateRequestDeleteStateData)
			if !ok {
...
func (m *DeleteShareGroupStateRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem DeleteShareGroupStateRequestDeleteStateData
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem DeleteShareGroupStateRequestDeleteStateData
				// TopicId
				if version >= 0 && version <= 999 {
				...
				if version >= 0 && version <= 999 {
					// Decode array using ArrayDecoder
					decoder := func(data []byte) (interface{}, int, error) {
						var elem DeleteShareGroupStateRequestPartitionData
						elemR := bytes.NewReader(data)
						// Partition
						if version >= 0 && version <= 999 {
					...
					if isFlexible {
						...
						for i := int32(0); i < length; i++ {
							...
							var elemBuf bytes.Buffer
							elemW := &elemBuf
							var tempElem DeleteShareGroupStateRequestPartitionData
							// Partition
							if version >= 0 && version <= 999 {
						...
						tempElem.Partitions = make([]DeleteShareGroupStateRequestPartitionData, len(decoded))
						for i, item := range decoded {
							tempElem.Partitions[i] = item.(DeleteShareGroupStateRequestPartitionData)
						}
					} else {
			...
			// Convert []interface{} to typed slice
			m.Topics = make([]DeleteShareGroupStateRequestDeleteStateData, len(decoded))
			for i, item := range decoded {
				m.Topics[i] = item.(DeleteShareGroupStateRequestDeleteStateData)
			}
		} else {
			length, err := protocol.ReadInt32(r)
			if err != nil {
...
// writeTaggedFields writes tagged fields for DeleteShareGroupStateRequestDeleteStateData.
func (m *DeleteShareGroupStateRequestDeleteStateData) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for DeleteShareGroupStateRequestDeleteStateData.
func (m *DeleteShareGroupStateRequestDeleteStateData) readTaggedFields(r io.Reader, version int16) error {
...
// writeTaggedFields writes tagged fields for DeleteShareGroupStateRequestPartitionData.
func (m *DeleteShareGroupStateRequestPartitionData) writeTaggedFields(w io.Writer, version int16) error {
	var taggedFieldsCount int
	var taggedFieldsBuf bytes.Buffer
...
// readTaggedFields reads tagged fields for DeleteShareGroupStateRequestPartitionData.
func (m *DeleteShareGroupStateRequestPartitionData) readTaggedFields(r io.Reader, version int16) error {
	// Read tagged fields count
	count, err := protocol.ReadVaruint32(r)
	if err != nil {
		return err
