...
// Round-trips a fully populated AlterClientQuotasRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestAlterClientQuotasRequestRoundTrip(t *testing.T) {
	in := &AlterClientQuotasRequest{
		Entries: &[]AlterClientQuotasRequestEntrie{AlterClientQuotasRequestEntrie{
			Entity: &[]AlterClientQuotasRequestEntrieEntity{AlterClientQuotasRequestEntrieEntity{
				EntityType: reqPtr("x"),
				EntityName: reqPtr("x"),
			}},
			Ops: &[]AlterClientQuotasRequestEntrieOp{AlterClientQuotasRequestEntrieOp{
				Key:    reqPtr("x"),
				Value:  1,
				Remove: true,
			}},
	...
	inNulls := &AlterClientQuotasRequest{
		Entries: &[]AlterClientQuotasRequestEntrie{AlterClientQuotasRequestEntrie{
			Entity: &[]AlterClientQuotasRequestEntrieEntity{AlterClientQuotasRequestEntrieEntity{
				EntityType: reqPtr("x"),
				EntityName: nil,
	...
	for v := int16(0); v <= 1; v++ {
		...
		_ = (&AlterClientQuotasRequest{}).PrettyPrint()
	}
}
