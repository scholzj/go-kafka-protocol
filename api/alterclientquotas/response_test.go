...
// Round-trips a fully populated AlterClientQuotasResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestAlterClientQuotasResponseRoundTrip(t *testing.T) {
	in := &AlterClientQuotasResponse{
		ThrottleTimeMs: 1,
		Entries: &[]AlterClientQuotasResponseEntrie{AlterClientQuotasResponseEntrie{
			ErrorCode:    1,
			ErrorMessage: resPtr("x"),
			Entity: &[]AlterClientQuotasResponseEntrieEntity{AlterClientQuotasResponseEntrieEntity{
				EntityType: resPtr("x"),
				EntityName: resPtr("x"),
			}},
		}},
	...
	inNulls := &AlterClientQuotasResponse{
		ThrottleTimeMs: 1,
		Entries: &[]AlterClientQuotasResponseEntrie{AlterClientQuotasResponseEntrie{
			ErrorCode:    1,
			ErrorMessage: nil,
	...
	for v := int16(0); v <= 1; v++ {
		...
		_ = (&AlterClientQuotasResponse{}).PrettyPrint()
	}
}
