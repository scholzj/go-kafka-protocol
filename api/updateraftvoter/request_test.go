...
// Round-trips a fully populated UpdateRaftVoterRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestUpdateRaftVoterRequestRoundTrip(t *testing.T) {
	in := &UpdateRaftVoterRequest{
		ClusterId:          reqPtr("x"),
		CurrentLeaderEpoch: 1,
		VoterId:            1,
		VoterDirectoryId:   uuid.UUID{},
		Listeners: &[]UpdateRaftVoterRequestListener{UpdateRaftVoterRequestListener{
			Name: reqPtr("x"),
			Host: reqPtr("x"),
			Port: 1,
		}},
	...
	inNulls := &UpdateRaftVoterRequest{
		ClusterId:          nil,
		CurrentLeaderEpoch: 1,
		VoterId:            1,
		VoterDirectoryId:   uuid.UUID{},
	...
	for v := int16(0); v <= 0; v++ {
		...
		_ = (&UpdateRaftVoterRequest{}).PrettyPrint()
	}
}
