...
func (req *ConsumerGroupDescribeRequest) Write(w io.Writer) error {
	// GroupIds (versions: 0+)
	if req.GroupIds == nil {
		return fmt.Errorf("ConsumerGroupDescribeRequest.GroupIds must not be nil in version %d", req.ApiVersion)
	}
...
func (req *ConsumerGroupDescribeRequest) Read(request *protocol.Request) error {
	if request == nil || request.Body == nil {
		return fmt.Errorf("ConsumerGroupDescribeRequest.Read: request or its body is nil")
	}
...
func (req *ConsumerGroupDescribeRequest) PrettyPrint() string {
	w := bytes.NewBuffer([]byte{})

	fmt.Fprintf(w, "    -> ConsumerGroupDescribeRequest:\n")
