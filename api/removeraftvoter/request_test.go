...
// Round-trips a fully populated RemoveRaftVoterRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestRemoveRaftVoterRequestRoundTrip(t *testing.T) {
	in := &RemoveRaftVoterRequest{
		ClusterId:        reqPtr("x"),
		VoterId:          1,
	...
	inNulls := &RemoveRaftVoterRequest{
		ClusterId:        nil,
		VoterId:          1,
		VoterDirectoryId: uuid.UUID{},
	}
	...
	for v := int16(0); v <= 0; v++ {
		...
		_ = (&RemoveRaftVoterRequest{}).PrettyPrint()
	}
}
