...
// Round-trips a fully populated ListPartitionReassignmentsRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestListPartitionReassignmentsRequestRoundTrip(t *testing.T) {
	in := &ListPartitionReassignmentsRequest{
		TimeoutMs: 1,
		Topics: &[]ListPartitionReassignmentsRequestTopic{ListPartitionReassignmentsRequestTopic{
			Name:             reqPtr("x"),
	...
	inNulls := &ListPartitionReassignmentsRequest{
		TimeoutMs: 1,
		Topics:    nil,
	}
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &ListPartitionReassignmentsRequest{}
		...
		_ = (&ListPartitionReassignmentsRequest{}).PrettyPrint()
	}
}
