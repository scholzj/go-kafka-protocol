...
// Round-trips a fully populated DescribeConfigsRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeConfigsRequestRoundTrip(t *testing.T) {
	in := &DescribeConfigsRequest{
		Resources: &[]DescribeConfigsRequestResource{DescribeConfigsRequestResource{
			ResourceType:      1,
			ResourceName:      reqPtr("x"),
			ConfigurationKeys: &[]string{"x"},
		}},
	...
	inNulls := &DescribeConfigsRequest{
		Resources: &[]DescribeConfigsRequestResource{DescribeConfigsRequestResource{
			ResourceType:      1,
			ResourceName:      reqPtr("x"),
			ConfigurationKeys: nil,
		}},
		IncludeSynonyms:      true,
		IncludeDocumentation: true,
	}
	...
	for v := int16(1); v <= 4; v++ {
		...
		_ = (&DescribeConfigsRequest{}).PrettyPrint()
	}
}
