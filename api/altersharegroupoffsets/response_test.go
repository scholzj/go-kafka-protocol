...
// Round-trips a fully populated AlterShareGroupOffsetsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestAlterShareGroupOffsetsResponseRoundTrip(t *testing.T) {
	in := &AlterShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   resPtr("x"),
		Responses: &[]AlterShareGroupOffsetsResponseResponse{AlterShareGroupOffsetsResponseResponse{
			TopicName: resPtr("x"),
			TopicId:   uuid.UUID{},
			Partitions: &[]AlterShareGroupOffsetsResponseResponsePartition{AlterShareGroupOffsetsResponseResponsePartition{
				PartitionIndex: 1,
				ErrorCode:      1,
				ErrorMessage:   resPtr("x"),
			}},
	...
	inNulls := &AlterShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   nil,
		Responses: &[]AlterShareGroupOffsetsResponseResponse{AlterShareGroupOffsetsResponseResponse{
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &AlterShareGroupOffsetsResponse{}
		...
		_ = (&AlterShareGroupOffsetsResponse{}).PrettyPrint()
	}
}
