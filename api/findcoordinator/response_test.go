...
// Round-trips a fully populated FindCoordinatorResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestFindCoordinatorResponseRoundTrip(t *testing.T) {
	in := &FindCoordinatorResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		...
		Host:           resPtr("x"),
		Port:           1,
		Coordinators: &[]FindCoordinatorResponseCoordinator{FindCoordinatorResponseCoordinator{
			Key:          resPtr("x"),
			NodeId:       1,
			Host:         resPtr("x"),
			Port:         1,
	...
	inNulls := &FindCoordinatorResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   nil,
		NodeId:         1,
	...
	for v := int16(0); v <= 6; v++ {
		...
		_ = (&FindCoordinatorResponse{}).PrettyPrint()
	}
}
