...
type FindCoordinatorRequest struct {
	...
	KeyType         int8      // The coordinator key type. (group, transaction, share). (versions: 1+)
	CoordinatorKeys *[]string // The coordinator keys. (versions: 4+)
...
func (req *FindCoordinatorRequest) Write(w io.Writer) error {
	...
	if req.ApiVersion <= 3 {
		if req.Key == nil {
			return fmt.Errorf("FindCoordinatorRequest.Key must not be nil in version %d", req.ApiVersion)
		}
	...
	if req.ApiVersion >= 4 {
		if req.CoordinatorKeys == nil {
			return fmt.Errorf("FindCoordinatorRequest.CoordinatorKeys must not be nil in version %d", req.ApiVersion)
		}
		if isRequestFlexible(req.ApiVersion) {
