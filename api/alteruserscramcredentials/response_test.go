...
// Round-trips a fully populated AlterUserScramCredentialsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestAlterUserScramCredentialsResponseRoundTrip(t *testing.T) {
	in := &AlterUserScramCredentialsResponse{
		ThrottleTimeMs: 1,
		Results: &[]AlterUserScramCredentialsResponseResult{AlterUserScramCredentialsResponseResult{
			User:         resPtr("x"),
			ErrorCode:    1,
	...
	inNulls := &AlterUserScramCredentialsResponse{
		ThrottleTimeMs: 1,
		Results: &[]AlterUserScramCredentialsResponseResult{AlterUserScramCredentialsResponseResult{
			User:         resPtr("x"),
			ErrorCode:    1,
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &AlterUserScramCredentialsResponse{}
		...
		_ = (&AlterUserScramCredentialsResponse{}).PrettyPrint()
	}
}
