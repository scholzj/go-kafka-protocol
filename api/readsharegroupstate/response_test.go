...
// Round-trips a fully populated ReadShareGroupStateResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestReadShareGroupStateResponseRoundTrip(t *testing.T) {
	in := &ReadShareGroupStateResponse{
		Results: &[]ReadShareGroupStateResponseResult{ReadShareGroupStateResponseResult{
			TopicId: uuid.UUID{},
			Partitions: &[]ReadShareGroupStateResponseResultPartition{ReadShareGroupStateResponseResultPartition{
				Partition:    1,
				ErrorCode:    1,
				ErrorMessage: resPtr("x"),
				StateEpoch:   1,
				StartOffset:  1,
				StateBatches: &[]ReadShareGroupStateResponseResultPartitionStateBatche{ReadShareGroupStateResponseResultPartitionStateBatche{
					FirstOffset:   1,
					LastOffset:    1,
					DeliveryState: 1,
					DeliveryCount: 1,
	...
	inNulls := &ReadShareGroupStateResponse{
		Results: &[]ReadShareGroupStateResponseResult{ReadShareGroupStateResponseResult{
			TopicId: uuid.UUID{},
			Partitions: &[]ReadShareGroupStateResponseResultPartition{ReadShareGroupStateResponseResultPartition{
				Partition:    1,
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &ReadShareGroupStateResponse{}
		...
		_ = (&ReadShareGroupStateResponse{}).PrettyPrint()
	}
}
