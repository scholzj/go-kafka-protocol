...
func (m *DescribeProducersRequest) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(DescribeProducersRequestTopicRequest)
			if !ok {
...
func (m *DescribeProducersRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem DescribeProducersRequestTopicRequest
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem DescribeProducersRequestTopicRequest
				// Name
				if version >= 0 && version <= 999 {
					if isFlexible {
			...
			m.Topics = make([]DescribeProducersRequestTopicRequest, len(decoded))
			for i, item := range decoded {
				m.Topics[i] = item.(DescribeProducersRequestTopicRequest)
			}
		} else {
...
// writeTaggedFields writes tagged fields for DescribeProducersRequestTopicRequest.
func (m *DescribeProducersRequestTopicRequest) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for DescribeProducersRequestTopicRequest.
func (m *DescribeProducersRequestTopicRequest) readTaggedFields(r io.Reader, version int16) error {
