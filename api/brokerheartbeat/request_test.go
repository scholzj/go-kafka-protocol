...
// Round-trips a fully populated BrokerHeartbeatRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestBrokerHeartbeatRequestRoundTrip(t *testing.T) {
	in := &BrokerHeartbeatRequest{
		...
		WantFence:             true,
		WantShutDown:          true,
		OfflineLogDirs:        &[]uuid.UUID{uuid.UUID{}},
	...
	inNulls := &BrokerHeartbeatRequest{
		BrokerId:              1,
		BrokerEpoch:           1,
		CurrentMetadataOffset: 1,
		WantFence:             true,
	...
	for v := int16(0); v <= 2; v++ {
		...
		_ = (&BrokerHeartbeatRequest{}).PrettyPrint()
	}
}
