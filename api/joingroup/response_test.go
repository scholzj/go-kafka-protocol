...
// Round-trips a fully populated JoinGroupResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestJoinGroupResponseRoundTrip(t *testing.T) {
	in := &JoinGroupResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		GenerationId:   1,
		ProtocolType:   resPtr("x"),
		ProtocolName:   resPtr("x"),
		Leader:         resPtr("x"),
		SkipAssignment: true,
		MemberId:       resPtr("x"),
		Members: &[]JoinGroupResponseMember{JoinGroupResponseMember{
			MemberId:        resPtr("x"),
	...
	for v := int16(0); v <= 9; v++ {
		...
		_ = (&JoinGroupResponse{}).PrettyPrint()
	}
}
