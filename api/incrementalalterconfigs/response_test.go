...
// Round-trips a fully populated IncrementalAlterConfigsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestIncrementalAlterConfigsResponseRoundTrip(t *testing.T) {
	in := &IncrementalAlterConfigsResponse{
		ThrottleTimeMs: 1,
		Responses: &[]IncrementalAlterConfigsResponseResponse{IncrementalAlterConfigsResponseResponse{
			ErrorCode:    1,
	...
	inNulls := &IncrementalAlterConfigsResponse{
		ThrottleTimeMs: 1,
		Responses: &[]IncrementalAlterConfigsResponseResponse{IncrementalAlterConfigsResponseResponse{
			ErrorCode:    1,
			ErrorMessage: nil,
	...
	for v := int16(0); v <= 1; v++ {
		{
			...
			out := &IncrementalAlterConfigsResponse{}
		...
		_ = (&IncrementalAlterConfigsResponse{}).PrettyPrint()
	}
}
