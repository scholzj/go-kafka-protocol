...
// Round-trips a fully populated CreateAclsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestCreateAclsResponseRoundTrip(t *testing.T) {
	in := &CreateAclsResponse{
		ThrottleTimeMs: 1,
		Results: &[]CreateAclsResponseResult{CreateAclsResponseResult{
			ErrorCode:    1,
	...
	for v := int16(1); v <= 3; v++ {
		...
		_ = (&CreateAclsResponse{}).PrettyPrint()
	}
}
