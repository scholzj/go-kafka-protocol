...
func (m *BeginQuorumEpochRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		if isFlexible {
			...
			m.Topics = make([]BeginQuorumEpochRequestTopicData, length)
			for i := int32(0); i < length; i++ {
				...
				if version >= 0 && version <= 999 {
					...
					if isFlexible {
						...
						length = int32(lengthUint - 1)
						m.Topics[i].Partitions = make([]BeginQuorumEpochRequestPartitionData, length)
						for i := int32(0); i < length; i++ {
							// PartitionIndex
							if version >= 0 && version <= 999 {
								val, err := protocol.ReadInt32(r)
	...
	if version >= 1 && version <= 999 {
		...
		if isFlexible {
			...
			length = int32(lengthUint - 1)
			m.LeaderEndpoints = make([]BeginQuorumEpochRequestLeaderEndpoint, length)
			for i := int32(0); i < length; i++ {
				// Name
				if version >= 1 && version <= 999 {
					if isFlexible {
