...
func (m *ListGroupsResponse) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(ListGroupsResponseListedGroup)
			if !ok {
			...
			if version >= 5 && version <= 999 {
				if isFlexible {
					if err := protocol.WriteCompactString(elemW, structItem.GroupType); err != nil {
						return nil, err
					}
				} else {
					if err := protocol.WriteString(elemW, structItem.GroupType); err != nil {
						return nil, err
					}
				}
			}
...
func (m *ListGroupsResponse) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem ListGroupsResponseListedGroup
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem ListGroupsResponseListedGroup
				// GroupId
				if version >= 0 && version <= 999 {
					if isFlexible {
				...
				if version >= 5 && version <= 999 {
					if isFlexible {
						if err := protocol.WriteCompactString(elemW, tempElem.GroupType); err != nil {
							return err
						}
					} else {
						if err := protocol.WriteString(elemW, tempElem.GroupType); err != nil {
							return err
						}
					}
				}
			...
			m.Groups = make([]ListGroupsResponseListedGroup, len(decoded))
			for i, item := range decoded {
				m.Groups[i] = item.(ListGroupsResponseListedGroup)
			}
		} else {
...
// writeTaggedFields writes tagged fields for ListGroupsResponseListedGroup.
func (m *ListGroupsResponseListedGroup) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for ListGroupsResponseListedGroup.
func (m *ListGroupsResponseListedGroup) readTaggedFields(r io.Reader, version int16) error {
