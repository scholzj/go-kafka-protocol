...
// Round-trips a fully populated DeleteAclsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDeleteAclsResponseRoundTrip(t *testing.T) {
	in := &DeleteAclsResponse{
		ThrottleTimeMs: 1,
		FilterResults: &[]DeleteAclsResponseFilterResult{DeleteAclsResponseFilterResult{
			ErrorCode:    1,
			ErrorMessage: resPtr("x"),
			MatchingAcls: &[]DeleteAclsResponseFilterResultMatchingAcl{DeleteAclsResponseFilterResultMatchingAcl{
				ErrorCode:      1,
				ErrorMessage:   resPtr("x"),
				ResourceType:   1,
	...
	for v := int16(1); v <= 3; v++ {
		...
		_ = (&DeleteAclsResponse{}).PrettyPrint()
	}
}
