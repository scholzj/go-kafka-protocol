...
// Round-trips a fully populated LeaveGroupRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestLeaveGroupRequestRoundTrip(t *testing.T) {
	in := &LeaveGroupRequest{
		GroupId:  reqPtr("x"),
		MemberId: reqPtr("x"),
		Members: &[]LeaveGroupRequestMember{LeaveGroupRequestMember{
			MemberId:        reqPtr("x"),
	...
	for v := int16(0); v <= 5; v++ {
		...
		_ = (&LeaveGroupRequest{}).PrettyPrint()
	}
}
