...
// Round-trips a fully populated LeaveGroupResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestLeaveGroupResponseRoundTrip(t *testing.T) {
	in := &LeaveGroupResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		Members: &[]LeaveGroupResponseMember{LeaveGroupResponseMember{
			MemberId:        resPtr("x"),
	...
	for v := int16(0); v <= 5; v++ {
		...
		_ = (&LeaveGroupResponse{}).PrettyPrint()
	}
}
