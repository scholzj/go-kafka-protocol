...
func (m *LeaveGroupResponse) Write(w io.Writer, version int16) error {
	...
	if version >= 3 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(LeaveGroupResponseMemberResponse)
			if !ok {
...
func (m *LeaveGroupResponse) Read(r io.Reader, version int16) error {
	...
	if version >= 3 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem LeaveGroupResponseMemberResponse
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem LeaveGroupResponseMemberResponse
				// MemberId
				if version >= 3 && version <= 999 {
					if isFlexible {
			...
			m.Members = make([]LeaveGroupResponseMemberResponse, len(decoded))
			for i, item := range decoded {
				m.Members[i] = item.(LeaveGroupResponseMemberResponse)
			}
		} else {
...
// writeTaggedFields writes tagged fields for LeaveGroupResponseMemberResponse.
func (m *LeaveGroupResponseMemberResponse) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for LeaveGroupResponseMemberResponse.
func (m *LeaveGroupResponseMemberResponse) readTaggedFields(r io.Reader, version int16) error {
