...
// Round-trips a fully populated DeleteShareGroupOffsetsResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDeleteShareGroupOffsetsResponseRoundTrip(t *testing.T) {
	in := &DeleteShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   resPtr("x"),
		Responses: &[]DeleteShareGroupOffsetsResponseResponse{DeleteShareGroupOffsetsResponseResponse{
			TopicName:    resPtr("x"),
			TopicId:      uuid.UUID{},
			ErrorCode:    1,
			ErrorMessage: resPtr("x"),
	...
	inNulls := &DeleteShareGroupOffsetsResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   nil,
		Responses: &[]DeleteShareGroupOffsetsResponseResponse{DeleteShareGroupOffsetsResponseResponse{
	...
	for v := int16(0); v <= 0; v++ {
		{
			...
			out := &DeleteShareGroupOffsetsResponse{}
		...
		_ = (&DeleteShareGroupOffsetsResponse{}).PrettyPrint()
	}
}
