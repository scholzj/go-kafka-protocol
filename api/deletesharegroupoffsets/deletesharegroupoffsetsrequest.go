...
func (m *DeleteShareGroupOffsetsRequest) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic)
			if !ok {
...
func (m *DeleteShareGroupOffsetsRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic
				// TopicName
				if version >= 0 && version <= 999 {
					if isFlexible {
						val, err := protocol.ReadCompactString(r)
			...
			m.Topics = make([]DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic, len(decoded))
			for i, item := range decoded {
				m.Topics[i] = item.(DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic)
			}
		} else {
...
// writeTaggedFields writes tagged fields for DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic.
func (m *DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic.
func (m *DeleteShareGroupOffsetsRequestDeleteShareGroupOffsetsRequestTopic) readTaggedFields(r io.Reader, version int16) error {
