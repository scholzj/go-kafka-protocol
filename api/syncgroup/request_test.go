...
// Round-trips a fully populated SyncGroupRequest through Write/Read/Write at every
...
func TestSyncGroupRequestRoundTrip(t *testing.T) {
	in := &SyncGroupRequest{
		...
		ProtocolName:    reqPtr("x"),
		Assignments: &[]SyncGroupRequestAssignment{SyncGroupRequestAssignment{
			MemberId:   reqPtr("x"),
			Assignment: &[]byte{1, 2, 3},
		}},
	}
	...
	for v := int16(0); v <= 5; v++ {
		...
		_ = (&SyncGroupRequest{}).PrettyPrint()
	}
}
