...
// Round-trips a fully populated DescribeClientQuotasRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestDescribeClientQuotasRequestRoundTrip(t *testing.T) {
	in := &DescribeClientQuotasRequest{
		Components: &[]DescribeClientQuotasRequestComponent{DescribeClientQuotasRequestComponent{
			EntityType: reqPtr("x"),
			MatchType:  1,
			Match:      reqPtr("x"),
		}},
	...
	inNulls := &DescribeClientQuotasRequest{
		Components: &[]DescribeClientQuotasRequestComponent{DescribeClientQuotasRequestComponent{
			EntityType: reqPtr("x"),
			MatchType:  1,
			Match:      nil,
	...
	for v := int16(0); v <= 1; v++ {
		{
			...
			out := &DescribeClientQuotasRequest{}
		...
		_ = (&DescribeClientQuotasRequest{}).PrettyPrint()
	}
}
