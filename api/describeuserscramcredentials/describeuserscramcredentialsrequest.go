...
func (m *DescribeUserScramCredentialsRequest) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(DescribeUserScramCredentialsRequestUserName)
			if !ok {
		...
		items := make([]interface{}, len(m.Users))
		for i := range m.Users {
			items[i] = m.Users[i]
		}
		if m.Users == nil {
...
func (m *DescribeUserScramCredentialsRequest) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem DescribeUserScramCredentialsRequestUserName
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem DescribeUserScramCredentialsRequestUserName
				// Name
				if version >= 0 && version <= 999 {
					if isFlexible {
			...
			m.Users = make([]DescribeUserScramCredentialsRequestUserName, len(decoded))
			for i, item := range decoded {
				m.Users[i] = item.(DescribeUserScramCredentialsRequestUserName)
			}
		} else {
...
// writeTaggedFields writes tagged fields for DescribeUserScramCredentialsRequestUserName.
func (m *DescribeUserScramCredentialsRequestUserName) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for DescribeUserScramCredentialsRequestUserName.
func (m *DescribeUserScramCredentialsRequestUserName) readTaggedFields(r io.Reader, version int16) error {
