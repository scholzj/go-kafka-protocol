...
// Round-trips a fully populated ElectLeadersRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestElectLeadersRequestRoundTrip(t *testing.T) {
	in := &ElectLeadersRequest{
		ElectionType: 1,
		TopicPartitions: &[]ElectLeadersRequestTopicPartition{ElectLeadersRequestTopicPartition{
			Topic:      reqPtr("x"),
			Partitions: &[]int32{1},
		}},
		TimeoutMs: 1,
	...
	for v := int16(0); v <= 2; v++ {
		...
		_ = (&ElectLeadersRequest{}).PrettyPrint()
	}
}
