...
// Round-trips a fully populated ElectLeadersResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestElectLeadersResponseRoundTrip(t *testing.T) {
	in := &ElectLeadersResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ReplicaElectionResults: &[]ElectLeadersResponseReplicaElectionResult{ElectLeadersResponseReplicaElectionResult{
			Topic: resPtr("x"),
			PartitionResult: &[]ElectLeadersResponseReplicaElectionResultPartitionResult{ElectLeadersResponseReplicaElectionResultPartitionResult{
				PartitionId:  1,
				ErrorCode:    1,
	...
	for v := int16(0); v <= 2; v++ {
		...
		_ = (&ElectLeadersResponse{}).PrettyPrint()
	}
}
