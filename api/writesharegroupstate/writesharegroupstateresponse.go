...
func (m *WriteShareGroupStateResponse) Write(w io.Writer, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		encoder := func(item interface{}) ([]byte, error) {
			...
			structItem, ok := item.(WriteShareGroupStateResponseWriteStateResult)
			if !ok {
...
func (m *WriteShareGroupStateResponse) Read(r io.Reader, version int16) error {
	...
	if version >= 0 && version <= 999 {
		...
		decoder := func(data []byte) (interface{}, int, error) {
			var elem WriteShareGroupStateResponseWriteStateResult
		...
		if isFlexible {
			...
			for i := int32(0); i < length; i++ {
				...
				var elemBuf bytes.Buffer
				elemW := &elemBuf
				var tempElem WriteShareGroupStateResponseWriteStateResult
				// TopicId
				if version >= 0 && version <= 999 {
				...
				if version >= 0 && version <= 999 {
					// Decode array using ArrayDecoder
					decoder := func(data []byte) (interface{}, int, error) {
						var elem WriteShareGroupStateResponsePartitionResult
						elemR := bytes.NewReader(data)
						// Partition
						if version >= 0 && version <= 999 {
					...
					if isFlexible {
						...
						for i := int32(0); i < length; i++ {
							...
							var elemBuf bytes.Buffer
							elemW := &elemBuf
							var tempElem WriteShareGroupStateResponsePartitionResult
							// Partition
							if version >= 0 && version <= 999 {
						...
						tempElem.Partitions = make([]WriteShareGroupStateResponsePartitionResult, len(decoded))
						for i, item := range decoded {
							tempElem.Partitions[i] = item.(WriteShareGroupStateResponsePartitionResult)
						}
					} else {
			...
			// Convert []interface{} to typed slice
			m.Results = make([]WriteShareGroupStateResponseWriteStateResult, len(decoded))
			for i, item := range decoded {
				m.Results[i] = item.(WriteShareGroupStateResponseWriteStateResult)
			}
		} else {
			length, err := protocol.ReadInt32(r)
			if err != nil {
...
// writeTaggedFields writes tagged fields for WriteShareGroupStateResponseWriteStateResult.
func (m *WriteShareGroupStateResponseWriteStateResult) writeTaggedFields(w io.Writer, version int16) error {
...
// readTaggedFields reads tagged fields for WriteShareGroupStateResponseWriteStateResult.
func (m *WriteShareGroupStateResponseWriteStateResult) readTaggedFields(r io.Reader, version int16) error {
...
// writeTaggedFields writes tagged fields for WriteShareGroupStateResponsePartitionResult.
func (m *WriteShareGroupStateResponsePartitionResult) writeTaggedFields(w io.Writer, version int16) error {
	var taggedFieldsCount int
	var taggedFieldsBuf bytes.Buffer
...
// readTaggedFields reads tagged fields for WriteShareGroupStateResponsePartitionResult.
func (m *WriteShareGroupStateResponsePartitionResult) readTaggedFields(r io.Reader, version int16) error {
	// Read tagged fields count
	count, err := protocol.ReadVaruint32(r)
	if err != nil {
		return err
