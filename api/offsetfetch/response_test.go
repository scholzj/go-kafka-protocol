...
// Round-trips a fully populated OffsetFetchResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestOffsetFetchResponseRoundTrip(t *testing.T) {
	in := &OffsetFetchResponse{
		ThrottleTimeMs: 1,
		Topics: &[]OffsetFetchResponseTopic{OffsetFetchResponseTopic{
			Name: resPtr("x"),
			Partitions: &[]OffsetFetchResponseTopicPartition{OffsetFetchResponseTopicPartition{
				PartitionIndex:       1,
				CommittedOffset:      1,
		...
		ErrorCode: 1,
		Groups: &[]OffsetFetchResponseGroup{OffsetFetchResponseGroup{
			GroupId: resPtr("x"),
			Topics: &[]OffsetFetchResponseGroupTopic{OffsetFetchResponseGroupTopic{
				Name:    resPtr("x"),
				TopicId: uuid.UUID{},
				Partitions: &[]OffsetFetchResponseGroupTopicPartition{OffsetFetchResponseGroupTopicPartition{
					PartitionIndex:       1,
					CommittedOffset:      1,
					CommittedLeaderEpoch: 1,
					Metadata:             resPtr("x"),
	...
	for v := int16(1); v <= 10; v++ {
		...
		_ = (&OffsetFetchResponse{}).PrettyPrint()
	}
}
