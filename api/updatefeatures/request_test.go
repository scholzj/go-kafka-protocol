...
// Round-trips a fully populated UpdateFeaturesRequest through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestUpdateFeaturesRequestRoundTrip(t *testing.T) {
	in := &UpdateFeaturesRequest{
		TimeoutMs: 1,
		FeatureUpdates: &[]UpdateFeaturesRequestFeatureUpdate{UpdateFeaturesRequestFeatureUpdate{
			Feature:         reqPtr("x"),
			MaxVersionLevel: 1,
			AllowDowngrade:  true,
			UpgradeType:     1,
	...
	for v := int16(0); v <= 1; v++ {
		...
		_ = in.PrettyPrint()
		_ = (&UpdateFeaturesRequest{}).PrettyPrint()
	}
}
