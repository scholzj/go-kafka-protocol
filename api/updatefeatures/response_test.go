...
// Round-trips a fully populated UpdateFeaturesResponse through Write/Read/Write at every
// valid protocol version and checks the re-encoded bytes match.
func TestUpdateFeaturesResponseRoundTrip(t *testing.T) {
	in := &UpdateFeaturesResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   resPtr("x"),
		Results: &[]UpdateFeaturesResponseResult{UpdateFeaturesResponseResult{
			Feature:      resPtr("x"),
			ErrorCode:    1,
			ErrorMessage: resPtr("x"),
		}},
	...
	inNulls := &UpdateFeaturesResponse{
		ThrottleTimeMs: 1,
		ErrorCode:      1,
		ErrorMessage:   nil,
		Results: &[]UpdateFeaturesResponseResult{UpdateFeaturesResponseResult{
	...
	for v := int16(0); v <= 2; v++ {
		...
		_ = (&UpdateFeaturesResponse{}).PrettyPrint()
	}
}
